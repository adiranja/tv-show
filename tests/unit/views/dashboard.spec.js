...
describe('In DashBoard Component', () => {
  ...
  it("created assigns shows, popularShows", () => {
    expect(dashboardWrapper.vm.shows).not.toBe(undefined);
    expect(dashboardWrapper.vm.popularShows).not.toBe(undefined);
  });

  it("getting show list works", (done) => {
    expect(dashboardWrapper.vm.shows.length).toBe(0);
    expect(dashboardWrapper.vm.popularShows.length).toBe(0);
    moxios.wait(function () {
      ...
      request
        ...
        .then(function () {
          ...
          expect(dashboardWrapper.vm.popularShows.length).toBe(2);
